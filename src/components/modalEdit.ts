...
import { getActiveUser } from "@/utils/getLocalStorage";
import { ITask } from "@/interfaces/taskManager";
import { TOnUpdate } from "@/types/taskManager";

// Define types for ITask and the onUpdate function
...
class ModalEdit {
    ...
    constructor(task: ITask, onUpdate: TOnUpdate) {
        this.task = task;
    ...
    open(): void {
        ...
        if (!mainElement) return; // Safety check to ensure the element exists
        ...
        form.addEventListener("submit", (event) => {
            ...
            this.handleUpdate(this.task.id, sanitizedTitle, prioritySelect.value as ITask["priority"], deadlineInput.value);
        });
    ...
    handleUpdate(id: string, title: string, priority: ITask["priority"], deadline: string): void {
        const activeUser = getActiveUser();
        this.onUpdate({
            id, userId: activeUser.id, title, priority, deadline,
            isCompleted: false
        });
        this.close();
    }
